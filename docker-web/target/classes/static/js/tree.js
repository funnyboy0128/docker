...
var svg = d3.select("body").append("svg").attr("width",
		...
		height + padding.top + padding.bottom).append("g").attr("transform",
...
d3.json("learn.json", function(error, root) {
	...
	function redraw(source) {
		...
		// 1. 节点的 Enter 部分的处理办法
		var enterNodes = nodeEnter.append("g").attr("class", "node").attr(
		...
		/**设置节点样式**/
		/*enterNodes.append("circle").attr("r", 0).style("fill", function(d) {
		...
		updateNodes.select("circle").attr("r", 8).style("fill", function(d) {
			return d._children ? "lightsteelblue" : "#fff";
		});
		...
		// 1. 连线的 Enter 部分的处理办法
		linkEnter.insert("path", ".node").attr("class", "link").attr("d",
				function(d) {
					var o = {
						x : source.x0,
						y : source.y0
